...
hanging_object_model = YOLO("/home/yasir/retina/Abnormal_Behaviour/yolo11s_hanging_material.pt")
face_detector_model = YOLO("/home/yasir/retina/Abnormal_Behaviour/yolov11s-face.pt")
hand_detector_model = YOLO("/home/yasir/retina/Abnormal_Behaviour/yolov11s-hand.pt")
...
face_conf_threshold = CONFIG["face_detection_confidence"]
hand_conf_threshold = CONFIG["hand_on_face"]["hand_conf_threshold"]
...
def detect(model, detection_confidence, frame, annotated_frame):
    """
    Runs YOLO11 detection on the frame, returns annotated frame and detections for class "bottle".
    """
    results = model.predict(frame, conf=detection_confidence, classes=[0])  # run detection
...
def main(video_path, alert_folder):
    ...
    if camera_misalignment_flag:
        ...
        for frame in original_frame_list:
            ...
            annotated_frame, boxB = detect(bottle_model, bottle_conf_threshold, frame, frame.copy())
            ...
            annotated_frame, boxA = detect(face_detector_model, face_conf_threshold, frame, frame.copy())
        ...
        if len(values) >= bottle_on_face_threshold:
            ...
            for frame in original_frame_list:
                ...
                # annotated_frame, boxA = detect(face_detector_model, face_conf_threshold, frame, frame.copy())
                results = hand_detector_model.predict(frame, conf=hand_conf_threshold, classes=[0])
            ...
            if len(values) >= hand_on_face_threshold:
                ...
                for frame in original_frame_list:
                    ...
                    no_face_flag = True                    
                    annotated_frame, box = detect(face_detector_model, face_conf_threshold, frame, frame.copy())
