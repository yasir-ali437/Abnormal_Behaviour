...
model = YOLO("/home/yasir/retina/bottle_detection_training/runs/detect/bottle_yolo11/weights/best.pt")  # use the nano version, change if needed
...
def main(video_number):    
    ...
    while True:
        ...
        if frame_count % 15 == 0:
            ...
            annotated_frame, boxB = detect(model, frame)
    ...
    if len(values) >= bottle_on_face_threshold:
        output_folder_path = os.path.join("/data1/yasir/Data/Safety Hazard/", "Eat & Drink",video_path.split('/')[-1].split('.')[0])
    else:
